-- The Chats-list preview leaned on italic to distinguish a deleted
-- message's placeholder text from a real one — but the custom font has no
-- italic weight loaded, so it silently rendered upright. Prefix with an
-- emoji instead, matching the existing 📷/📎 attachment-preview pattern.
...
returns table (
  ...
  select cv.id, cv.kind,
         ...
         coalesce(lm.body, case when lm.attachment_type = 'image' then '📷 photo'
                                 ...
                                 when lm.id is not null then '🗑️ this message was deleted' end) as last_body,
